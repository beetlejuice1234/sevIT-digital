...
export function FloatingWhatsApp() {
  const buttonRef = useMagneticEffect<HTMLAnchorElement>({ radius: 60, strength: 0.5 });

  useEffect(() => {
    // Add a slight delay so it doesn't pop in immediately
    gsap.fromTo(
      buttonRef.current,
      { opacity: 0, scale: 0.5, y: 50 },
      { opacity: 1, scale: 1, y: 0, duration: 0.6, ease: 'back.out(1.5)', delay: 1.5 }
    );
  }, [buttonRef]);
  ...
  return (
    ...
    >
      ...
      {/* Pulse effect rings */}
      <div className="absolute inset-0 rounded-full border-2 border-[#25D366] animate-ping opacity-20" style={{ animationDuration: '2s' }} />
    </a>
  );
}
