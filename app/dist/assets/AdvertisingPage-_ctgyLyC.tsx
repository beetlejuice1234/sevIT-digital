...
function MobileSwipeCarousel({ children, activeColor = 'bg-red-500' }: { children: React.ReactNode[], activeColor?: string }) {
...
// What We Actually Do — ad service breakdown card
...
// Honest problem/solution visual
function WhyAdsFailSection() {
  const reasons = [
    {
      problem: 'Targeting everyone',
      fix: 'Narrow, intent-based audiences',
      icon: Target,
    },
    {
      problem: 'Set-it-and-forget-it campaigns',
      fix: 'Ongoing weekly optimization',
      icon: Zap,
    },
    {
      problem: 'No creative testing',
      fix: 'Multiple ad variants, data picks the winner',
      icon: Layers,
    },
    {
      problem: 'Clicks without conversions',
      fix: 'Landing pages built around the ad message',
      icon: MousePointer,
    },
    {
      problem: 'Vanity metrics (likes, reach)',
      fix: 'Focus on leads, sales, and real ROI',
      icon: BarChart2,
    },
  ];
  ...
  return (
    <>
      <div className="hidden md:block space-y-4">
        {reasons.map((r, i) => {
          ...
          return (
            <div
              key={i}
              className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-0 bg-white/[0.02] border border-white/10 rounded-2xl p-5 group hover:border-red-500/30 transition-all duration-300"
            >
      ...
      <MobileSwipeCarousel activeColor="bg-red-500">
        {reasons.map((r, i) => {
          const Icon = r.icon;
          return (
            <div key={i} className="flex flex-col items-center justify-center text-center bg-white/[0.02] border border-white/10 rounded-2xl p-6 h-full w-full gap-4">
              <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center">
                <Icon className="w-6 h-6 text-white/40" />
              </div>
              <div>
                <p className="text-white/40 text-sm line-through decoration-red-500/60 mb-2">{r.problem}</p>
                <ArrowRight className="w-5 h-5 text-red-500 mx-auto transform rotate-90 mb-2" />
                <p className="text-white text-base font-bold">{r.fix}</p>
              </div>
            </div>
          );
        })}
...
function AdvertisingPage() {
  ...
  const services = [
    {
      ...
      description:
        'People searching for what you sell are already warm leads. We make sure your ad is the first thing they see — and that clicking it actually leads somewhere worth clicking.',
      bullets: [
    ...
    },
    {
      ...
      description:
        'Meta and TikTok have absurd targeting power — most brands just use it badly. We build audiences around the people most likely to actually care about your product.',
      bullets: [
    ...
    },
    {
      ...
      description:
        "You'll always know exactly where your money is going and what it's returning. No confusing dashboards — just clear, honest reporting and a direct line to us.",
      bullets: [
  ...
  const processSteps = [
    {
      ...
      description:
        "Before we write a single ad, we want to understand who your customers are, what makes them buy, and what your competitors are doing wrong. Good ads start with good homework.",
    },
    {
      number: '02',
      title: 'We Build the Campaign',
      description:
        'We set up your campaign structure, write the copy, design the creative direction, and configure targeting — all with a clear goal in mind, not just to spend your budget.',
    },
    {
      number: '03',
      title: 'We Test, Then Scale',
      description:
        'We run multiple versions of your ads to see what resonates, then pour more budget into the winners and cut what isn\'t earning its keep. No guessing — just data.',
    },
    {
      number: '04',
      title: 'We Keep You in the Loop',
      description:
        'You get regular updates on what\'s performing, what we\'re changing, and why. If something isn\'t working, we tell you before you have to ask.',
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white overflow-x-hidden">
      ...
      {/* WHY ADS FAIL */}
      <section className="why-section relative py-24 px-6 lg:px-12 z-10 border-t border-white/10">
      ...
      <section className="process-section relative py-24 px-6 lg:px-12 z-10 border-t border-white/10">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl lg:text-5xl font-black mt-2">From Day One to Results</h2>
            <p className="text-white/40 mt-4 max-w-lg mx-auto text-sm leading-relaxed">
          ...
          <div className="hidden md:block space-y-4">
            {processSteps.map((step, i) => (
              <div
                key={i}
                className="process-step opacity-0 flex gap-6 bg-white/[0.02] border border-white/10 rounded-2xl p-6 hover:border-amber-500/30 transition-all duration-300 group"
              >
          ...
          {/* Mobile Carousel */}
          <MobileSwipeCarousel activeColor="bg-amber-500">
            {processSteps.map((step, i) => (
              <div
                key={i}
