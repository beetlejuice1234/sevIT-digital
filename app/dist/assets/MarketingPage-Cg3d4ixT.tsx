...
function MobileSwipeCarousel({ children, activeColor = 'bg-emerald-500' }: { children: React.ReactNode[], activeColor?: string }) {
    ...
    <div className="md:hidden block w-full">
      ...
      <div className="flex justify-center gap-1.5 mt-2">
        {React.Children.map(children, (_, i) => (
          <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === activeIndex ? "w-6 " + activeColor : 'w-1.5 bg-white/20'}`} />
        ))}
      </div>
    </div>
  );
...
// What growth marketing actually is — simplified visual explainer
function GrowthExplainer() {
  const pillars = [
    {
      icon: TrendingUp,
      label: 'Get Found',
      detail: 'Showing up in search results when people are already looking for what you offer.',
    },
    {
      icon: Users,
      label: 'Get Chosen',
      detail: "Turning website visitors into actual customers by making your site easy to trust and use.",
    },
    {
      icon: RefreshCw,
      label: 'Get Them Back',
      detail: 'Keeping past customers engaged so they buy again — the most efficient revenue you can make.',
    },
    {
      icon: BarChart3,
      label: 'Keep Improving',
      detail: "Every campaign teaches us something. We use that to make the next one better.",
    },
  ];
  ...
  return (
    <>
      <div className="hidden md:grid grid-cols-1 sm:grid-cols-2 gap-4">
        {pillars.map((p, i) => {
          ...
          return (
            <div
              key={i}
              className="flex gap-4 bg-white/[0.02] border border-white/10 rounded-2xl p-5 hover:border-emerald-500/30 transition-all duration-300 group"
            >
      ...
      <MobileSwipeCarousel activeColor="bg-emerald-500">
        {pillars.map((p, i) => {
          const Icon = p.icon;
          return (
            <div
              key={i}
              className="flex flex-col items-center text-center bg-white/[0.02] border border-white/10 rounded-2xl p-6 hover:border-emerald-500/30 transition-all duration-300 group h-full justify-center gap-3"
            >
              <div className="w-12 h-12 rounded-xl bg-emerald-500/20 flex items-center justify-center shrink-0">
                <Icon className="w-6 h-6 text-emerald-500" />
              </div>
              <div>
                <h4 className="font-bold text-base mb-2 group-hover:text-emerald-400 transition-colors">{p.label}</h4>
                <p className="text-white/45 text-sm leading-relaxed">{p.detail}</p>
              </div>
            </div>
          );
        })}
...
function MarketingPage() {
  ...
  const services = [
    {
      ...
      description:
        "When someone searches for what you offer, you want to be there. SEO is the long game — it takes time to build, but once it's working, it brings in customers without you paying per click.",
      bullets: [
    ...
    },
    {
      ...
      description:
        "Getting traffic is only half the battle. If people land on your site and leave without doing anything, something's off. We find out what and fix it.",
      bullets: [
    ...
    },
    {
      ...
      description:
        "Email is still one of the highest-ROI marketing channels — because you own the list. We set up flows and campaigns that stay in touch with your customers without being annoying.",
      bullets: [
  ...
  const processSteps = [
    {
      ...
      description:
        "We start by looking at what you already have — your site, your traffic, your existing content. No point building on a broken foundation.",
    },
    {
      number: '02',
      title: 'Identify the Real Opportunities',
      description:
        "Not every business needs the same thing. We figure out which channels and tactics will actually move the needle for your specific situation, and prioritize those.",
    },
    {
      number: '03',
      title: 'Build and Execute',
      description:
        "We put the plan into action — whether that's fixing your site structure, creating content, setting up email flows, or running experiments on your landing pages.",
    },
    {
      number: '04',
      title: 'Measure, Learn, Repeat',
      description:
        "Growth marketing is an ongoing loop, not a one-time project. We track what's working, cut what isn't, and keep iterating until the numbers are consistently moving in the right direction.",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white overflow-x-hidden">
      ...
      {/* HERO */}
      <section className="relative min-h-[85vh] flex items-center justify-center px-6 lg:px-12 pt-32 pb-40 z-10">
        <div
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          ...
          <p className="hero-line text-xl text-white/50 max-w-2xl mx-auto mb-4 opacity-0 leading-relaxed">
          ...
          <p className="hero-line text-base text-white/35 max-w-xl mx-auto mb-10 opacity-0">
          ...
          <div className="hero-line opacity-0">
            <Link
              to="/#chat"
              className="inline-flex items-center gap-3 px-8 py-4 bg-emerald-500 text-white rounded-full font-bold uppercase tracking-wider hover:bg-white hover:text-black transition-all group"
            >
      ...
      <section className="explainer-section relative py-24 px-6 lg:px-12 z-10 border-t border-white/10">
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <span className="text-sm text-emerald-400 uppercase tracking-wider">The Short Version</span>
              <h2 className="text-4xl lg:text-5xl font-black mt-3 mb-5">
      ...
      <section className="process-section relative py-24 px-6 lg:px-12 z-10 border-t border-white/10">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl lg:text-5xl font-black mt-2">No mysteries. Just a clear process.</h2>
          ...
          <div className="hidden md:block space-y-4">
            {processSteps.map((step, i) => (
              <div
                key={i}
                className="process-step opacity-0 flex gap-6 bg-white/[0.02] border border-white/10 rounded-2xl p-6 hover:border-emerald-500/30 transition-all duration-300 group"
              >
          ...
          <MobileSwipeCarousel activeColor="bg-emerald-500">
            {processSteps.map((step, i) => (
              <div
                key={i}
                className="flex flex-col items-center text-center bg-white/[0.02] border border-white/10 rounded-2xl p-6 h-full w-full gap-4 group"
              >
                <div className="w-14 h-14 rounded-xl bg-emerald-500/20 flex items-center justify-center shrink-0">
                  <span className="text-xl font-black text-emerald-500">{step.number}</span>
                </div>
      ...
      <section className="relative py-32 px-6 lg:px-12 z-10 border-t border-white/10 overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          ...
          <p className="text-lg text-white/50 mb-3 leading-relaxed">
          ...
          <p className="text-sm text-white/30 mb-10 uppercase tracking-widest">
            Free conversation. Zero obligation.
          </p>
          <Link
            to="/#chat"
            className="inline-flex items-center gap-3 px-10 py-5 bg-emerald-500 text-white rounded-full font-bold text-lg uppercase tracking-wider hover:bg-white hover:text-black transition-all group"
          >
