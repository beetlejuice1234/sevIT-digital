...
function MobileSwipeCarousel({ children, activeColor = 'bg-pink-500' }: { children: React.ReactNode[], activeColor?: string }) {
    ...
    <div className="min-h-screen bg-[#050505] text-white overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center px-6 lg:px-12 pt-20 z-10">
        <div className="absolute inset-0 pointer-events-none" 
          style={{ background: 'radial-gradient(ellipse at 50% 30%, rgba(236, 72, 153, 0.15) 0%, transparent 60%)' }} 
        />

        <div className="relative z-10 max-w-6xl mx-auto text-center">
          {/* Label */}
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-pink-500/10 border border-pink-500/30 rounded-full mb-10">
          ...
          <div className="hero-cta opacity-0">
            <Link 
              to="/#chat" 
              className="inline-flex items-center gap-3 px-10 py-5 bg-pink-500 text-white rounded-full font-bold text-lg uppercase tracking-wider hover:bg-white hover:text-black transition-all duration-300 group"
            >
      ...
      <section className="pillars-section relative py-32 px-6 lg:px-12 z-10 border-t border-white/10">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-20">
            ...
            <p className="text-2xl sm:text-3xl text-pink-500 font-bold mt-2 uppercase tracking-wider">
          ...
          <div className="hidden lg:grid grid-cols-3 gap-8">
            {pillars.map((pillar, index) => {
              ...
              return (
                <div 
                  key={index}
                  className="pillar-card group relative opacity-0 block"
                >
                  <div 
          ...
          {/* Mobile Swipe Pillars */}
          <MobileSwipeCarousel activeColor="bg-pink-500">
            {pillars.map((pillar, index) => {
              const Icon = pillar.icon;
              return (
                <div 
                  key={index}
                  className="relative h-full p-8 lg:p-10 rounded-3xl bg-white/[0.03] border border-white/10 overflow-hidden text-left"
                >
                  <div className="relative w-14 h-14 rounded-2xl flex items-center justify-center mb-6" style={{ background: `${pillar.color}20` }}>
                    <Icon className="w-7 h-7" style={{ color: pillar.color }} />
                  </div>
                  <span className="text-[10px] sm:text-xs uppercase tracking-[0.2em] mb-3 block font-bold" style={{ color: pillar.color }}>{pillar.subtitle}</span>
                  <h3 className="text-2xl sm:text-3xl font-black leading-[0.95] mb-4 whitespace-pre-line">{pillar.title}</h3>
                  <div className="text-white/70 text-sm sm:text-base leading-relaxed whitespace-pre-line">{pillar.description}</div>
                  <div className="absolute bottom-0 left-0 right-0 h-[3px]" style={{ background: `linear-gradient(90deg, transparent, ${pillar.color}, transparent)` }} />
                </div>
              );
            })}
          </MobileSwipeCarousel>
      ...
      <section className="method-section relative py-32 px-6 lg:px-12 z-10 border-t border-white/10">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-16">
            ...
            <p className="text-3xl sm:text-4xl text-pink-500 font-bold uppercase tracking-wider">
              METHODOLOGY
            </p>
          ...
          <div className="text-center mb-20">
            ...
            <p className="text-3xl font-black text-pink-500 mt-4 uppercase tracking-widest">
          ...
          <div className="hidden md:grid grid-cols-3 gap-8">
            {methodologyPoints.map((point, index) => {
              ...
              return (
                <div 
                  key={index}
                  className="method-point text-center p-8 rounded-2xl bg-white/[0.02] border border-white/10 hover:border-pink-500/30 hover:bg-white/[0.04] transition-all opacity-0"
                >
            ...
            <MobileSwipeCarousel activeColor="bg-pink-500">
              {methodologyPoints.map((point, index) => {
                ...
                return (
                  <div 
                    key={index}
                    className="text-center p-8 rounded-2xl bg-white/[0.02] border border-white/10 h-full w-full flex flex-col justify-center items-center"
                  >
      ...
      <section className="relative py-32 px-6 lg:px-12 z-10 border-t border-white/10 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          <p className="text-pink-500 font-bold uppercase tracking-widest mb-12">
          ...
          <Link 
            to="/#chat" 
            className="inline-flex items-center gap-3 px-12 py-6 bg-pink-500 text-white rounded-full font-bold text-xl uppercase tracking-wider hover:bg-white hover:text-black transition-all duration-300 group"
          >
